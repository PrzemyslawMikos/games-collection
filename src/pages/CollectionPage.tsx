...
function EntryRow({ entry, onEdit, onDelete }: { entry: GameEntry; onEdit: () => void; onDelete: () => void }) {
  return <div className="entry-row"><span className="entry-type entry-owned">Egzemplarz</span><div className="entry-main"><strong>{entry.platform}</strong><span>{entry.version || 'Wersja podstawowa'} · {conditionLabel[entry.condition]}</span></div><span className={`pill pill-${entry.completion}`}>{completionLabel[entry.completion]}</span><span className="entry-price">{formatMoney(entry.price, entry.currency)}</span><button className="icon-button" type="button" aria-label="Edytuj egzemplarz" onClick={onEdit}><Pencil size={15} /></button><button className="icon-button danger" type="button" aria-label="Usuń egzemplarz" onClick={onDelete}><Trash2 size={15} /></button></div>
}
